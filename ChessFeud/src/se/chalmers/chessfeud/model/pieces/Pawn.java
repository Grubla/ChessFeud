...
/**
 * The piece Knight. Reprecents the Pawns on the chessborad. Handles the logic
 * for the Pawns movement and what team the pawn is on.
 * 
 * @author Arvid
 * 
...
public class Pawn extends Piece {
	...
	/**
	 * A method that finds out all moves the piece can do from the current
	 * position if the board was empty.
	 * 
	 * @param p
	 *            the Pawns current Position
	 * @return posList A list that contains Lists with all moves in all
	 *         direction
	 */
	@Override
	...
	/*
	 * Takes the current position, and depending on which team the pawn is on
	 * returns all the possible moves the piece can do, including the diagonal
	 * moves. If the pawn hasn't moved yet, the list will also contain the first
	 * double move which a pawn can make on its first turn.
	 * 
	 * @param px the pawns x-Position value
	 * 
	 * @param py the pawns y-Position value
	 * 
	 * @param team an integer that shows which team the pawn is on, 0 = white, 1
	 * = black
	 * 
	...
	private List<Position> moveDirection(int px, int py, int team) {
		...
		for (int i = 0; i < xIntList.size(); i++) {
			moveList.add(new Position(px + xIntList.get(i), py
					+ yIntList.get(i)));
		}
		return moveList;
